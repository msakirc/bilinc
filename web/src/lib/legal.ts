/**
 * Legal document text + metadata for Bilinç (web).
 *
 * Source of truth: docs/legal/*.md (Turkish drafts). These are TASLAK (draft)
 * texts and MUST be reviewed by a licensed Turkish lawyer before production.
 * [PLACEHOLDER] fields and the cross-border-transfer (KVKK m.9) consent wording
 * must be finalized. Kept in sync with mobile/src/config/legal.ts.
 */
...
export const LEGAL_URLS = {
  ...
  kvkk: '/yasal/kvkk',
  isletmeDogrulama: '/yasal/isletme-dogrulama', // PLACEHOLDER — not published yet
} as const;
...
// Exact Turkish consent copy surfaced on the register form.
